/*
 * RELIS — drivers/pci/pci.c
 * Top-tier PCI bus enumeration: Recursive bridge scanning, 64-bit BAR decoding,
 * and global device tracking.
 */
#include "pci.h"
#include "arch/io.h"
...
/* ── BAR Decoding (Supports 64-bit MMIO) ─────────────────────────────────── */
    ...
    for (int i = 0; i < bar_limit; i++) {
        ...
        if (bar_lo & 1) {
            ...
            if (type == PCI_BAR_TYPE_MMIO32) {
                ...
                i++; // 64-bit BAR consumes two registers
            }
        }
    }
}
...
void pci_scan_bus(uint8_t bus) {
    for (uint8_t slot = 0; slot < 32; slot++) {
        uint32_t id = pci_read32(bus, slot, 0, PCI_VENDOR_ID);
        ...
        if (header_type & PCI_HEADER_TYPE_MULTIFUNC) {
            for (uint8_t func = 0; func < 8; func++) {
                if (pci_read32(bus, slot, func, PCI_VENDOR_ID) != 0xFFFFFFFF) {
