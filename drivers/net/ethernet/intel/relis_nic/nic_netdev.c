...
void nic_rx_init(void) {
    ...
    // 3. Program the E1000 Receive Registers
    ...
    // 4. Enable Receive
    nic_write_reg(E1000_RCTL, (1 << 1) | (1 << 4)); // Enable + Unicast promiscuous
...
void nic_rx_poll(void) {
    ...
    if (rx_descs[next_tail].status & 0x01) { // DD bit set
        uint16_t len = rx_descs[next_tail].length;
        uint8_t *packet = (uint8_t*)rx_buffers[next_tail];
